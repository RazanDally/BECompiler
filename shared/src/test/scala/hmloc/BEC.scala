  ...
  with TimeLimitedTests
{
  ...
  protected lazy val files: Seq[Path] = allFiles.filter { file =>
    validExt(file.ext) && filter(file.relativeTo(pwd)) && file.baseName.contains("Survey")
  }
  ...
  files.foreach { file =>
    ...
    val (libCtx, libDeclared): (typer.Ctx, Map[Str, typer.PolymorphicType]) = loadLibrary(BEC.libPath, typer, output(_))
    ctx = libCtx
